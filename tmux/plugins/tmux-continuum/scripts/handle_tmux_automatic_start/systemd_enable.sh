...
template() {
	...
	ExecStart=${tmux_exec_path} ${systemd_tmux_server_start_cmd}

	ExecStop=${HOME}/.tmux/plugins/tmux-resurrect/scripts/save.sh
	ExecStop=${tmux_exec_path} kill-server
	KillMode=none
...
main() {
	...
	local tmux_exec_path="$(get_tmux_option "${tmux_exec_path_option}" "${tmux_exec_path_default}" )"
